...
def pad_sents_char(sents, char_pad_token):
    ...
    sents_padded = []       # each element is a padded sentence with the same number of words
    sent_lens = [len(sent) for sent in sents]
    max_sent_lens = max(sent_lens)
    for sent in sents:
        sent_padded = []    # each element is a padded word with the same number of chars
        for word in sent[:max_sent_lens]:
        ...
        sent_padded = sent_padded + [[char_pad_token] * max_word_length] * (max_sent_lens - len(sent_padded))
...
def pad_sents(sents, pad_token):
    ...
    # max_len * corpus_size
    sents_padded = [sents[i] + [pad_token] * (max_len - sent_lens[i]) for i in range(corpus_size)]
