...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        word_ids = []       # sentence level: each element is a sentence
        for sent in sents:
            char_ids = []   # word level: each element is a word
            for word in sent:
                # chars for each word
                char_id = [self.start_of_word] + [self.char2id[c] for c in word] + [self.end_of_word]
                char_ids.append(char_id)
            word_ids.append(char_ids)
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_sents_char = pad_sents_char(char_ids, self.char2id['<pad>'])
        sents_var = torch.tensor(padded_sents_char, dtype=torch.long).permute(1, 0, 2).to(device)
        return sents_var
