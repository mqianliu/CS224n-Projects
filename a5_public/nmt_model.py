...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        if self.charDecoder is not None:
            ...
            target_chars = target_padded_chars[1:].contiguous().view(-1, max_word_len)  # TODO
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = pack_padded_sequence(X, lengths=source_lengths)  # omit the padding when feeding the sequence into RNN
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)  # pad 0s back in the output, (max_len, b, 2*h)
        enc_hiddens = enc_hiddens.transpose(0, 1)  # batch first: (b, max_len, 2*h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens)  # (b, seq_len, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # (b, seq_len, h) x (b, h, 1) -> (b, seq_len, 1) -> (b, seq_len)
        ...
        a_t = enc_hiddens.transpose(1, 2).bmm(alpha_t.unsqueeze(-1)).squeeze(-1)  # (b, 2h)
